    ...
    <main className={styles.page}>
      ...
      <section className={styles.compare}>
        ...
        <div className={styles.related}>
          {related.map((item) => <Link href={`/de/getraenke/${item.id}`} key={item.id}><span>{brandById[item.brandId]?.name}</span><strong>{item.name.replace(`${brandById[item.brandId]?.name} `, "")}</strong><b>{format(item.sugarPer100Ml)} g / 100 ml</b><ArrowRight size={16} /></Link>)}
        </div>
      </section>
    </main>
  );
